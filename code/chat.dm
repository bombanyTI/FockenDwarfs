proc/to_chat(target,message) //Proc that outputs messages to chat
	var/index = findtext(message, "я")
	while(index) //(This shit is for russian text)Пока в тексте есть "я" - бьенд продолжает заменять эти буквы на "&#255;"
...
/mob/living/verb/Say(message as text)
	...
	msg("<B>[name]</B> says, \"[message]\"")
	cantalk = 0; spawn(4) cantalk = 1
